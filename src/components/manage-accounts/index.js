...
export default function ManageAccounts() {
  ...
  async function handleSave(e) {
    ...
    if (data.success) {
      await getAllAccounts(); // Await for the accounts to refresh
  ...
  // Simplified toggle function for the form
  ...
  return (
    <div className="min-h-screen flex justify-center flex-col items-center relative">
        ...
        <ul className="flex  p-0 my-[25px]">
          {accounts && accounts.length
            ? accounts.map((item) => (
                <li
                  key={item._id}
                  className="max-w-[200px] min-w-[200px] w-[155px] cursor-pointer flex flex-col items-center gap-3 "
                  onClick={
                    showDeleteIcon
                      ? null
                      : () =>
                ...
                >
                  <div className="relative">
                    <img
                      src="https://th.bing.com/th/id/OIP.mIp1PimSjuheifox20urIgAAAA?pid=ImgDet&rs=1"
                      alt="Account Profile image"
                      className="max-w-[200px] rounded min-w-[64px] max-h-[200px] min-h-[84px] object-cover w-[155px] h-[155px]"
                    />
                    {showDeleteIcon ? (
                      <div
          ...
          {accounts && accounts.length < 4 ? (
            <li
              className="border text-black bg-[#e5b109] font-bold text-lg border-black max-w-[200px] rounded min-w-[64px] max-h-[200px] min-h-[84px] cursor-pointer flex justify-center items-center w-[155px] h-[155px]"
        ...
        {accounts && accounts.length > 0 ? (
          <div className="text-center">
            <span
              ...
              className="cursor-pointer border border-gray-100 tracking-wide inline-flex text-sm px-[1.5em] py-[0.5em] "
            >
              Manage Profile
            </span>
          </div>
      ...
      {/* Render the form based on showAccountForm */}
      {showAccountForm && (
        <AccountForm
          formData={formData}
