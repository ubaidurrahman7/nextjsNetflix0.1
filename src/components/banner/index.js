...
export default function Banner({ medias }) {
  ...
  return (
    <div
      className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[70hv] 
    lg:justify-end lg:pb-12 lg:pl-24 "
    >
      ...
      <h1 className="text-2xl md:text-4xl lg:text-7xl font-bold md:mt-4">
        {createRandomMedia?.title ||
          createRandomMedia?.name ||
          createRandomMedia?.orignal_name}
      </h1>
      ...
      <div className="flex space-x-3">
        ...
        <button
          className="cursor-pointer flex items-center gap-x-2 px-4
        rounded px5 py-1.5 text-sm font-semibold transition hover:opacity-75
         md:py-2.5 md:px-8 md:text-xl bg-[gray]/70 "
        >
          <IoMdInformationCircleOutline className="h-4 w-4  md:h-8 md:w-8 cursor-pointer" />
          More Info
        </button>
      </div>
    </div>
